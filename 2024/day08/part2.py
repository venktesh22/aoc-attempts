...
def solve_part2(input_data):
    ...
    for char, _ in all_chars.items():
        ...
        for (x1, y1), (x2, y2) in combinations(all_coords, 2):
            for direction in [(x2 - x1, y2 - y1), (x1 - x2, y1 - y2)]:
                i = 0
                while True:
                    nx, ny = (x2 if direction == (x2 - x1, y2 - y1) else x1) + i * direction[0], (y2 if direction == (x2 - x1, y2 - y1) else y1) + i * direction[1]
